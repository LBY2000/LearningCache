...
private:
  ...
  static const int kCasBufferCnt = 8;     // async, buffer safty
   static const int kEntryBufferCnt = 8;
...
public:
  ...
  void set_buffer(char *buffer){
    ...
    sibling_buffer = (char *)page_buffer + sizeof(LeafBucket) * kPageBufferCnt;
   // sibling_buffer = (char *)page_buffer + sizeof(kLeafPageSize) * kPageBufferCnt;
    entry_buffer = (char *)sibling_buffer + kLeafPageSize * kSiblingBufferCnt;
  //  entry_buffer = (char *)sibling_buffer + sizeof(kLeafPageSize) * kSiblingBufferCnt;
    *zero_64bit = 0;
  ...
  //  return page_buffer + (page_buffer_cur * sizeof(kLeafPageSize));
    return page_buffer + (page_buffer_cur * sizeof(LeafBucket));
  }
  ...
  char *get_sibling_buffer() {
    ...
    return sibling_buffer + (sibling_buffer_cur * kLeafPageSize);
 //    return sibling_buffer + (sibling_buffer_cur * sizeof(kLeafPageSize));
  }

  char *get_entry_buffer() { 
  entry_buffer_cur = (entry_buffer_cur + 1)%kEntryBufferCnt;
    return entry_buffer+ (entry_buffer_cur * kLeafPageSize); 
    }
};
