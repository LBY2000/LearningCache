...
//    std::atomic<int> cache_invalid_counter;
  ...
  void broadcast_new_root(GlobalAddress new_root_addr, int root_level);  //在update_new_root阶段进行broadcast_new_root
...
private:
  ...
  int16_t last_index;  //last_index是可用的最后一位的位置标识
  Key lowest;
  Key highest;
...
constexpr int kLeafCardinality =
    (kLeafPageSize - sizeof(Header) - sizeof(uint8_t) * 2 - 8) / sizeof(LeafEntry);//最终是leafEntry的数量。之前有padding所以扇出为4，后续改了以后就不是这样了
...
public:
  ...
  InternalPage(GlobalAddress left, const Key &key, GlobalAddress right,
    ...
    hdr.last_index = 0;//再次印证了last_index是最后一个有效entry的位置索引
